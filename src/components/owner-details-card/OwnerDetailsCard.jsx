...
const OwnerDetailsCard = ({
  ...
  handleClickMobileNumber // 👈 Added new prop for login popup
}) => {
      ...
      <div className="relative -mt-10 p-2 md:-mt-8 lg:-mt-8 lg:p-5">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 md:gap-4 md:mb-6 xl:flex-row xl:flex-nowrap w-full border-b newBorderColor pb-4">
          ...
          <div className="flex flex-col flex-grow justify-center md:justify-normal gap-2 sm:gap-6 w-full">
            <div className="self-center md:self-end"></div>
        ...
        {mobile && mobile !== "null" && (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              ...
              {/* ✅ Modified phone display logic */}
              {userCurrentId ? (
                // Logged-in → full number with call link
                <Link href={`tel:${mobile}`} className="leadColor text-sm">
                  {mobile}
                </Link>
              ) : (
                // Not logged-in → show masked & trigger login popup
                <button
                  type="button"
                  className="leadColor text-sm underline"
                  onClick={handleClickMobileNumber}
                >
                  {`${mobile?.slice(0, 6)}XXXXX`}
                </button>
              )}
            </div>
          </div>
        ...
        {(total_properties > 0 || total_projects > 0) && (
        ...
        {userCurrentId !== added_by && (
          <div className="flex justify-center gap-3">
            ...
            <button
              ...
              onClick={(e) =>
                router.push(
                  `/${locale}/agent-details/${customer_slug_id}/${(isProject && is_admin_listing === 1) || added_by === 0
                    ? `?is_admin=true`
                    : ""
                  }`
                )
