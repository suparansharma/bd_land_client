...
const LoginModal = ({ showLogin, setShowLogin }) => {
  ...
  const generateOTP = async (phoneNumber) => {
  try {
    // Firebase বা আপনার API কল এখানে দিন
    // 🔹 যদি আপনার ব্যাকএন্ড API দিয়ে OTP পাঠান:
    ...
    if (response.status) {
      ...
      setTimeLeft(120);
      setIsCounting(true);
      if (isDemo) setOTP(DemoOTP);
    } else {
      toast.error(response.message || t("otpFailed"));
    }
  } catch (error) {
    console.error("Error generating OTP:", error);
  ...
  //         console.log('isFirebaseOtp',isFirebaseOtp);
  //   return;
  ...
  try {
    ...
    if (isFirebaseOtp) {
      // if (!window.recaptchaVerifier) await generateRecaptcha();
      await generateOTP(value);
    } else if (isTwilioOtp) {
      await generateOTPWithTwilio(value);
    } else {
      // Use your API
      const response = await sendLoginOtpApi({
        mobile: phoneUtil.format(phoneNumber, "E.164"), // full number with +880 etc
...
const handleConfirm = async (e) => {
  ...
  try {
    ...
    // -------------------------------
      ...
      await handleTwilioOTPConfirm();

    // -------------------------------
    // API OTP flow (custom backend)
    // -------------------------------demo
    } else {
  ...
  //   }
  // };
// Resend OTP handler (without reCAPTCHA)
const handleResendOTP = async () => {
  setShowLoader(true);
  try {
    if (isEmailOtpEnabled) {
      ...
      // 🔹 Handle phone OTP resend
      if (isFirebaseOtp || isTwilioOtp) {
        try {
          
          const response = await sendLoginOtpApi({
            mobile: phonenum, // full number with +880...
          });

          if (response.status) {
            toast.success(response.message || t("otpSentSuccessfully"));
            if (isDemo) setOTP(DemoOTP);
            setTimeLeft(120);
            setIsCounting(true);
            setOTP(""); // Reset phone OTP input
        ...
        toast.error(t("otpMethodNotSupported"));
        setShowLoader(false);
      }
    }
  } catch (error) {
    ...
    toast.error(t(error?.message) || t("failedToSendOTP"));
    setShowLoader(false);
    // Reset counting states in case of error
    if (isEmailOtpEnabled) {
      setIsEmailCounting(false);
      setEmailTimeLeft(0);
    } else {
